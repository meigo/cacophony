...
>
> Cacophony started as an experiment in orchestrating many coding agents in parallel, on the bet that external coordination would unlock meaningful gains over a single agent working sequentially. In practice, those gains didn't materialize in the way I hoped: the overhead of managing worktrees, queues, retries, and hand-offs rarely paid for itself.
>
> In the meantime, frontier models and their native harnesses have moved remarkably fast. Modern agents now plan, branch, parallelize sub-tasks, and recover from failures on their own — and they do it better than an external scheduler wrapped around them can. The problem this project set out to solve is increasingly being solved one layer up, inside the model and its first-party tooling.
...
| ----------------- | -------- | ---------------------------- | ------------------------------------------------------------------ |
| `kind`            | string   | _required_                   | `"files"`, `"github"`, `"linear"`, or path to custom adapter       |
...
| Field                     | Type   | Default    | Description                                                                                                |
| ------------------------- | ------ | ---------- | ---------------------------------------------------------------------------------------------------------- |
| `command`                 | string | _required_ | Shell command to run. Supports Liquid: `{{prompt_file}}`, `{{workspace}}`, `{{identifier}}`, `{{attempt}}` |
