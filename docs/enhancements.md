...
### Folder-name leaks into framework detection (feature or bug?)
The brief subprocess is spawned with `cwd` set to `{projectRoot}/.cacophony/brief/session-xxx` (`src/brief.ts:245-271`). Claude Code and most CLI agents surface the working directory to the model, so the agent implicitly picks up the project folder name as a signal — e.g. a folder named `caco-run-test-react` biases framework suggestions toward React/Next even when the user's prompt says nothing about React. The prompt template never passes the folder name explicitly; it leaks through ambient cwd context. Arguably a useful prior (someone who names a folder `my-astro-blog` probably does want Astro) and arguably a footgun (scratch folders with misleading names mislead the brief). Fixes if it becomes a real problem: (a) run the brief from `os.tmpdir()` for full isolation — risk: some agents expect to run inside a git repo; (b) add a line to `BRIEF_TEMPLATE` instructing the agent to derive framework only from the user's prompt, not from directory names or filesystem inspection.
